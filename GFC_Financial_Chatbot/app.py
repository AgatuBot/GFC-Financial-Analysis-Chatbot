...
def detect_year(text):
    """Identify a year from 2023–2025."""
...
def simple_chatbot(user_query):
    ...
    if lower_query in [
        ...
        return (
            ...
            "Microsoft, Tesla, and Apple from 2023–2025. "
    ...
    if not is_financial_question:
        return (
            ...
            "growth, and financial comparisons from 2023–2025."
        )
    ...
    if len(companies) > 1 and metric and year:
        ...
        if not selected_data.empty:
            ...
            # Compare multiple companies when explicitly requested
            if "compare" in lower_query or "difference between" in lower_query:
                values = {}
                ...
                for company_name in companies:
                    ...
                    if not row.empty:
                        values[company_name] = row.iloc[0][metric]

                if len(values) > 1:
                    comparisons = []
                    company_names = list(values.keys())

                    for i in range(len(company_names)):
                        for j in range(i + 1, len(company_names)):
                            company_a = company_names[i]
                            company_b = company_names[j]
                            difference = values[company_a] - values[company_b]

                            if difference >= 0:
                                comparisons.append(
                                    f"{company_a} was {format_money(difference)} higher than {company_b}"
                                )
                            else:
                                comparisons.append(
                                    f"{company_b} was {format_money(abs(difference))} higher than {company_a}"
                                )

                    value_summary = "; ".join(
                        f"{name}: {format_money(value)}"
                        for name, value in values.items()
                    )

                    comparison_summary = "; ".join(comparisons)

                    return (
                        f"{metric_name.capitalize()} comparison in {year}: "
                        f"{value_summary}. {comparison_summary}."
                    )

            # Standard multi-company financial question
            responses = []
